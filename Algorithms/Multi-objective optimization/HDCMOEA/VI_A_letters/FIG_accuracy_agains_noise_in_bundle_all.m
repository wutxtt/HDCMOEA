%This script outputs the figure corresponding to Fig. 6 in the article       
% 
% AUTHOR
...
%
    ...
    Accuracy_Noisy_ds=zeros(3,Noise_limit);
    Accuracy_Noisy_ld=zeros(3,Noise_limit);
    Accuracy_Noisy_spr_2k=zeros(3,Noise_limit);
    Accuracy_Noisy_spr_10k=zeros(3,Noise_limit);
    Accuracy_Noisy_spr_100k=zeros(3,Noise_limit);
    Noisy_all=cell(Noise_limit,5); % store all statistics
    
    %i is the number of bit errors. Range from 1 to 15
    parfor i=1:Noise_limit
        ...
        for j=1:26
            ...
            LD(j,:)=round(sum(BUNDLE{j,2})/Bundle_size);
        ...
        for k=1:Noisy_recall
            ...
            for j=1:26 % number of letters
                ...
                Noise_dense(k,j)=item_memory_c(bin2com(repr), HGNc ); 
                
                %Noise_ld(k,j)=item_memory_overlap(pattern_nois', LD ); 
                Noise_ld(k,j)=item_memory_c(bin2com(pattern_nois'), LDc ); 
...
%%     
    ...
    ax = gca;
    ax.XTick = (1:15);
    xlabel('Distortion, bits','FontSize', 22)
